...
def process_track(
    ...
    处理一首歌：
      ...
      7. 移动文件 & 清理空目录
    """
    log_info(_("处理：{meta}").format(meta=meta))
...
def process_all(config: AppConfig) -> None:
    ...
    log_info(_("扫描音频文件：{extensions}").format(extensions=extensions_str))
    ...
    if total == 0:
        log_warn(_("未找到任何支持的音频文件（{extensions}）").format(extensions=extensions_str))
        return
    
    log_info(_("共找到 {total} 个音频文件").format(total=total))
